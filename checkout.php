...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $carrinho) {
  ...
  foreach ($carrinho as $id => $item) {
    ...
    // Format quantity display: 1.5kg or 2x
    $qtdDisplay = ($item['unidade'] === 'kg')
                  ? number_format($item['qtd'], 1, ',', '.') . "kg"
                  : number_format($item['qtd'], 0) . "x";

    $mensagem .= " • {$qtdDisplay} {$item['nome']} (R$ " . number_format($item['preco'], 2, ',', '.') . ")\n";
  }
  ...
  if ($checkTable->num_rows > 0) {
      ...
      if ($stmt) {
          ...
          if ($stmtItem) {
              foreach ($carrinho as $id => $item) {
                // Ensure float value for quantity
                $qtd = (float)$item['qtd'];
                $stmtItem->bind_param("iidd", $pedido_id, $id, $qtd, $item['preco']);
                $stmtItem->execute();
              }
              $stmtItem->close();
          }
          ...
          ?>
              <div class="preview-item">
                <div style="display:flex; justify-content:space-between;">
                    <span><strong><?= $qtdDisplay ?></strong> <?= htmlspecialchars($item['nome']) ?></span>
